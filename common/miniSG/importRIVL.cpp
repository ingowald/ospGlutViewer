...
#define NEXT_TOK strtok(nullptr, " \t\n\r")
                  ...
                  } else if (!childType.compare("string")) {
                    std::cout << " param " << childName << " = " << s << std::endl;
                  } else {
                    //error!
...
#undef NEXT_TOK
            ...
            xml::for_each_child_of(node,[&](const xml::Node &child){
                ...
                } else if (childType == "displacement") {
                  std::cout << "Mesh: ignoring displacement texture for now" << std::endl;
