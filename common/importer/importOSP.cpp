...
namespace ospray {
  namespace importer {
    ...
    template<typename T> std::pair<bool,T> getEnvVar(const char *name);

    template<>
    std::pair<bool,std::string> getEnvVar<std::string>(const char *name)
    {
      char *s = getenv(name);
      if (s) return std::make_pair(true,s);
